...
isConfirm = input('are you sure you want to deploy (y/n)?\n')
if isConfirm.lower() != 'y':
    print('stopped deployment')
    sys.exit(1)

with open('./utility/deploy_info.json') as info:
  deploy_server_info = json.load(info)

print('deploy info => ',deploy_server_info)

hostname = deploy_server_info['hostname']
port = deploy_server_info['port']
username = deploy_server_info['username']
password = deploy_server_info['password']

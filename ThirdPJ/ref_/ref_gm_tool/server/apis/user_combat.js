...
module.exports = function(app) {
    app.route('/api/user_combat').get(select).post(update); 
}
...
var select =  function(req,res) {
    ...
    let query = 'select * from user_combat where user_idx='+ userIdx +';';
    sql.execute_gamedb(worldIdx,query).then(result => {
        translate.setItemName(result,function(output){
            res.json({result:output});
        });
    }).catch(err =>{
        logger.error('user combat select error : ' + err.message );

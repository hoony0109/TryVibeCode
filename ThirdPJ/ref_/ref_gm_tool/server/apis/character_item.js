...
module.exports = function(app) {
    app.route('/api/character_item').get(select).post(update); 
}
...
var select = function(req,res) {
    ...
    let query = 'select item_type,item_id,item_value from char_inven_item where user_idx=' + userIdx + ' and char_idx='+ characterIdx +';';
    sql.execute_gamedb(worldIdx,query).then(result => {
        translate.setItemName(result,function(output){
            res.json({result:output});
        });   
    }).catch(err =>{
        logger.error('user character inven item select error : ' + err.message );

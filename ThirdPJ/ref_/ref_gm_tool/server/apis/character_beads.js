...
module.exports = function (app) {
    app.route('/api/character_beads').get(select).post(update);
}
...
var select = function (req, res) {
    ...
    let query = 'select slot_id,bead_id from char_inven_beads where char_idx=' + characterIdx + ';';
    sql.execute_gamedb(worldIdx, query).then(result => {
        translate.setItemName(result, function (output) {
            res.json({ result: output });
        });
    }).catch(err => {
        logger.error('user character beads select error : ' + err.message);

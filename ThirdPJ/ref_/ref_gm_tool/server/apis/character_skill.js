...
module.exports = function(app) {
    app.route('/api/character_skill').get(select).post(update); 
}
...
var select =  function(req,res) {
    ...
    let query = 'select skill_id,skill_value,select_type from char_skill where char_idx='+ characterIdx +';';
    sql.execute_gamedb(worldIdx,query).then(result => {
        translate.setSkillName(result,function(output){
            res.json({result:output});
        });
    }).catch(err =>{
        logger.error('user character skill select error : ' + err.message );
